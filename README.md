...
- **实验详情页**：以「实验」为第一公民——选手人数由游戏引擎 min/max 决定，在详情页开始对局、观战、登记训练、开对照、跨实验对比
...
- **模型蒸馏训练**：PEFT LoRA（无 GPU 走 CPU 快速验证）；训练产物可登记为 Ollama 选手并开对照实验
...
| 数据归档 | JSONL 本地文件 |
| 训练框架 | `poetry install --with training` 启用 PEFT LoRA（Transformers）；无 GPU 走 CPU 快速验证 |
...
# 选手配置在前端「选手配置」页创建，仓库不提供 YAML seed
...
2. 打开「选手配置」页，按引擎槽位数创建选手（斗地主需要 3 个）。首次为空，点空态按钮创建。
3. 后端启动后可访问 http://localhost:8000/api/v1/system/startup-check 查看警告。
4. 采集对局会校验所选配置的 API Key；未配置会被拒绝，而不是开局后静默失败。
5. 训练需 `cd server && poetry install --with training`。缺依赖时无法创建任务；无 GPU 自动走 CPU 快速验证。
6. 无 API 密钥时可在首页点「加载演示对局」体验观战与回放（演示局不挂实验）。

### 研究者主路径（推荐）

打开 http://localhost:5173 —— 首页即**实验列表**。

1. **创建选手配置**：到「选手配置」页为每个槽位建一份（模型 / 温度等）
2. **新建实验**：按引擎要求选择选手配置 + 目标局数 → 进入详情（**不会**自动开局，避免误消耗 API 密钥）
3. **开始对局 / 再开 n 局**：在详情页顶栏批量开局；进行中可观战，结束可回放
4. **登记并开训**：可训决策 > 0 时一键登记 ChatML 并创建训练任务（缺训练依赖则只登记）
5. **训练 · 模型仓库**：导出部署包 →（本机 GGUF + `ollama create`）→ **登记为选手**
6. **开对照实验**：详情「开对照实验」选挑战选手 + 与引擎槽位数相同的基线 → 新建实验后继续对局；「对比实验」并排看胜率 CI / 延迟 / Token

**基准测验**：新建实验时可选「基准测验（固定发牌）」模式，使用系统预置的 50 组发牌种子（`deal_seed`，最多 50 局），便于跨模型在相同牌面下公平对比；对比页会显示「基准测验」标记。

侧栏「试玩对局」可创建不挂实验的对局。实验详情仅含 **对局 / 选手表现** 两个 Tab；决策点、追踪、数据、训练在侧栏「数据与训练」，可用 `?experiment_id=` 深链，页面顶部有回实验的上下文条。完整说明见顶栏 **使用说明**（`/guide`）。
...
- 前端界面：http://localhost:5173（开发模式）
  - **Ink Lab 双壳**：默认进实验列表；`/experiments/:id` 为实验详情页；`/game/:id` 为全屏观战（GenericBoard）；`/guide` 为使用说明
- API 文档：http://localhost:8000/docs（Swagger UI）
- API 备选文档：http://localhost:8000/redoc（ReDoc）

### 配置实验参数

选手配置只保存在 **SQLite**，在前端「选手配置」页增删改。仓库没有 YAML seed：首次启动该表为空，必须先在 UI 里创建足够槽位的配置，才能新建实验。
...
侧栏「试玩对局」页可创建不挂实验的对局（1–50 局批量），数据同样进入决策点与看板。计入实验请从实验详情「开始对局」，便于按实验过滤与对照。
...
采样用 `temperature`、`top_p`、`max_tokens`；实验意图写在 `notes`（如「高温度对照组」）。
...
| 使用说明 | 前端 `/guide`（顶栏书本图标）：模块说明与流程图 |
| [架构设计](docs/ARCHITECTURE.md) | 系统架构、分层设计、核心流程 |
| [目录结构](docs/PROJECT_STRUCTURE.md) | 目录规划与模块职责 |
| [编码规范](docs/CODING_STANDARDS.md) | Python / TypeScript / Vue / 文案 i18n 规范 |
...
### 第三阶段：实验主线
- [x] 实验列表 / 实验详情页（顶栏主操作、结果摘要、对局与选手 Tab；决策/追踪/训练深链）
- [x] 按实验过滤决策导出与「登记并开训」
- [x] 训练产物登记为 Ollama 选手 + 开对照实验
- [x] 界面密度优化（关键指标条、紧凑列表、对比矩阵）+ 提示词 / 试玩对局 / 选手配置对齐；集中式 `/guide` 使用说明
...
>
> **真实训练**：`cd server && poetry install --with training`。产物为 `models/<task_id>/adapter/` LoRA 权重。无 GPU 时自动限制步数与样本（CPU 快速验证）。
>
> **本地部署**：
...
- [x] 一键 merge→GGUF→`ollama create`（训练页「推送到 Ollama」，需 `LLAMA_CPP_DIR`）
- [ ] 新增游戏引擎（三国杀等）
- [ ] 强化训练能力（如 PPO 等）
