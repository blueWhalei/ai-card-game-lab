...
- **Core** — framework-independent domain logic:
  ...
  - `eval/` — `rollout.py` scores candidate actions by determinized rollouts with common random numbers. Experiment-level **Scorer** plugins (`scorer.py` / `scorers.py`) turn persisted counts into named metrics (`train_usable`, `parser_success` first); `ExperimentService` overlays registered results onto summary fields from `protocol.scorer.eval_metric_ids`.
