...
- **Core** — framework-independent domain logic:
  ...
  - `eval/` — `rollout.py` scores candidate actions by determinized rollouts with common random numbers. Experiment-level **Scorer** plugins (`scorer.py` / `scorers.py`, plus `engine/doudizhu/scorers.py` for `role:landlord`) turn persisted counts into named metrics; `ExperimentService` overlays registered results onto summary fields from `protocol.scorer.eval_metric_ids`.
