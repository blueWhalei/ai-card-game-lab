...
The primary product object is an **experiment** (run): a fixed set of player configs plus a target game count. Record a hypothesis, run games, review decisions, compare experiments and save a conclusion from the experiment detail page. Training is optional under Analyze. Trial games (no experiment; zh: 试玩对局) still exist on `/game`.
...
CI: `npm run lint:ci` → `npm test` → `npm run build` (build already runs `type-check` for application code, unit tests and E2E tests via `tsconfig.test.json`).
...
- **Core** — framework-independent domain logic:
  ...
  - `ai/` — `LLMClient` ABC + `LLMClientFactory`. Two implementations: `OpenAICompatibleClient` (OpenAI, DashScope, DeepSeek, Kimi, Zhipu, Yi, Baichuan, MiniMax) and `OllamaClient`. Wired in `dependencies.py`. Streaming uses `stream_options: {"include_usage": true}`; final `StreamChunk` may carry `usage`. Clients accept `response_format` and degrade (drop `stream_options`, then `response_format`) only when a 400/422 explicitly identifies that field as unsupported; `OllamaClient` translates it to Ollama's `format`. There is **no** proactive per-provider capability probe or cache — only reactive degrade. Authentication/validation failures and rate limits are non-retryable within a decision: neither streaming fallback nor policy loops may turn them into another immediate request. Rate limits remain typed `AI_RATE_LIMIT_EXCEEDED` errors. Optional **VCR** wrapper (`core/ai/vcr.py`): `VCR_MODE=record|replay` stores/replays chat calls as JSONL under `data/vcr/` (match key = SHA-256 of provider + messages + model/sampling/`response_format`); `replay` miss raises `VcrMissError` (no silent live call). Default `off`. `core/ai/tools/` keeps game-agnostic helpers (e.g. `WinProbabilityTool`); Dou Dizhu analyzers are not re-exported from here.
  - `policy/` — `Policy` ABC: an async **event stream** (`ThinkingDelta` / `ToolCall` / `ToolResult` / `LlmRequest` / `LlmUsage` / `ActionChosen`) ending in exactly one `ActionChosen`. `LLMPolicy` owns single-shot ask-and-parse; `ToolLoopPolicy` (`tool_loop`) ReAct-calls engine tools then picks an id; `SearchAugmentedPolicy` (`search`) proposes ≤k candidates then scores via rollout. `HeuristicPolicy` / `FirstActionPolicy` / `RandomPolicy` are the non-LLM baselines. Live seats pick a policy via player-config `policy_kind` (`AIService`); preflight skips provider checks for baseline seats. A `Budget` caps LLM and tool calls; `PolicyContext` injects `EngineAdvisor`, `PromptSource`, optional `score_actions`, and the rng.
  - `eval/` — `rollout.py` scores candidate actions by determinized rollouts with common random numbers. Experiment-level **Scorer** plugins (`build_scorer_registry(game_type=…)`) cover every Dou Dizhu `eval_metric_id` (`train_usable`, `parser_success`, `latency_p50_p95`, `role:landlord`, `ev_loss`); `ExperimentService` overlays results onto summary from `protocol.scorer.eval_metric_ids`. Repo `eval_aggregates` supplies counts only (Dou Dizhu seat-as-landlord via `engine/doudizhu/seat_role_stats.py`); rate formulas live in Scorers. **Puzzle packs** (`puzzle.py` / `replay.py` / `perturb.py` + `PuzzleService`): extract high-spread decisions into `{data_dir}/puzzles/{pack_id}/`, score baselines offline, and **probe** consistency under legal-action / hand-card order shuffles (`POST .../probe`). Minimal Analyze UI at `/pipeline/puzzles`.
...
- **Config** — `app/config.py` (`pydantic-settings`) from env / project-root `.env`. Player configs live in SQLite and are created in the Player configs UI (`/experiment-configs`); each has `policy_kind` (`llm` default; `tool_loop` / `search` for LLM variants; or `heuristic` / `random` / `first` baseline — no API). There is no YAML seed.
...
| `/pipeline/puzzles` | `PuzzleView` (baseline packs; extract / run / probe) |
...
| `/settings` | `SettingsView` (read-only: providers, paths, storage, thinking-budget defaults, EV opponent / self-proxy, memory scope; preflight via `GET /api/v1/system/preflight`) |
...
- Collect accepts an optional `idempotency_key` (1–128 chars). Reserve games + seed protocol + request record in one transaction before launching any games; retries reuse the batch and only start `created` games. A reused key with a different count returns 409. Keep the frontend key until the response succeeds. See `docs/API_DESIGN.md`.
- Creating an experiment does **not** start games (avoids accidental API spend). Start the experiment from the detail page.
- Home (`/`): first-run checklist (configured provider → enough player configs → an experiment) until complete; **Load demo** seeds a main + control experiment pair (zero API) and opens `/experiments/exp_demo_main` at the verdict stage; trial replay game remains for `/game`. Import / compare live in a secondary menu; repo `examples/*.json` are importable experiment packs. Detail ⋯ **export** shares the experiment-pack format.
...
- Detail page phases, in the order `resolveStageId()` checks them: `empty` (no games — Start experiment, and say it costs API usage), `collecting` (progress number + watch), `harvest` (structural trainable count + Review all decisions, including excluded records), `verdict` (`StageVerdict`). Identity bar (back / name / status / ⋯) and the **archive** dialog (notebook, protocol, validation, clone/manifest) stay on every phase. The ⋯ menu carries the experiment-scoped Pipeline entries (decisions / data / training / traces); a phase only links out when that *is* the next step (e.g. `review_decisions`), so removing a phase block must not prune a deep link. Home list keeps **Compare several experiments**.
- The verdict phase shows: when `can_conclude`, one conclusion sentence from `delta.verdict_key` plus the Δ number; when not, the **headline is the evidence line** (`stage.evidence.*`) and Δ is a muted footnote — never a causal claim as the title. A support line (paired n + CI; when present also McNemar `paired_p` and bootstrap `paired_ci`) stays under the number. Scenario subscores are a `ExperimentScenarioBars` small-multiples row that only annotates the one notable gap when the claim is strong enough; Δ is **not** colored good/bad.
...
- Summary / compare expose eval metrics: role win rates, parser rate, train_usable, P50/P95 latency (from `rounds`), tokens/game, status counts, per-seat as-landlord win rate (needs `metadata.landlord_id`), plus `credibility` (decisive_n / CI width / low_power) and `scenario_scores` (bidding / playing / endgame / bomb: train_usable + parser). Collect CTA uses `GET /api/v1/system/preflight` (seat providers); Settings shows the same checks as a read-only machine-status page (keys stay in project-root `.env`), including thinking-budget defaults, `ev_loss_opponent_kind` / `ev_loss_self_proxy`, and `memory_scope`. UI copy comes from `preflight.*` by check `id` (not the backend `message`). Layout title for `/pipeline/*` is always `nav.analyze`. Experiment detail owns one identity bar with `HeaderToggles` (WorkbenchLayout skips its desktop toggles strip there). Collect confirms inline with a batch stepper; while `collecting`, Stop games is a ghost secondary on the stage and also in the ⋯ menu — not a second primary CTA. Player configs are a card roster (win rate is not colored good/bad).
- `GET /experiments/{id}` adds computed `timeline`, `validation` (control runs + `control_progress` + `validation_ready`), `next_step` (`review_decisions` after collection regardless of training availability; `collect_control` → control experiment collect; `review` + `action=stay` when a control is ready — stay on the detail verdict, do not jump to compare), `delta` (vs source or first control: landlord win-rate Δ, paired n, CI, optional `paired_p` / `paired_ci` from McNemar + paired bootstrap, `can_conclude` / `inconclusive_reason`, `verdict_key`, plus per-scenario train/parser Δ), and `benchmark` (`null` unless `collect_mode=benchmark`: seed coverage of the declared `deal_seeds`). Create freezes from Settings into `protocol.solver.thinking_budget`, `protocol.solver.memory` (`none` \| `per_experiment`), and `protocol.scorer.evaluator` (EV knobs including `opponent_kind` / `self_proxy`). New decisions store `game_phase=endgame` when any remaining hand has ≤8 cards.
...
- Finished games: step replay (play/pause, prev/next, speed). Current replay frame can show the same EV explain line as highlights via `GET /api/v1/games/{id}/decision-commentary` (not a board sticker; live frames skip this). Post-game **highlights** (3–5 moves from stored decision points: last play, blunder, bomb, parse fallback, endgame, high-branch — a `blunder` is `ev_loss >= BLUNDER_EV_LOSS`, and it outranks the others because it is the only reason derived from what a move was worth rather than what it looked like) on the result dialog and observer history panel; jump seeks replay and links to `/pipeline/decisions?game_id=&decision_id=`. When `evaluator_params.best_action_id` is present, the highlight line also shows AI vs EV-best vs loss.
- Demo (homepage): `POST /api/v1/system/seed-demo` → `{ experiment_id, game_id, created }`; homepage navigates to the experiment detail (verdict); `/game` still uses `game_id` for observer replay.

## Database (SQLite)

Schema lives in `app/database.py`. Tables:

`experiments`, `games` (nullable `experiment_id`), `rounds`, `datasets`, `training_tasks` (nullable `experiment_id`), `prompt_templates`, `traces`, `spans`, `decision_points` (`train_usable`, `quality_score`, `ev_loss`, `policy_kind`, `tool_calls`, `annotation`), `experiment_configs` (`policy_kind`: `llm` / `tool_loop` / `search` / `heuristic` / `random` / `first`).
...
decision `annotation` = migration 4; `experiment_memory` = migration 5; durable `experiment_collect_requests` = migration 6 →
`SCHEMA_VERSION = 6`).

JSONL under `data/games/{YYYY-MM-DD}/` uses the path frozen in `games.data_file`, including across UTC midnight. The writer restores that path at startup of each game; old multi-day fragments are merged for replay. SQLite stores the index and structured decision/trace records.
...
2. Override `capability` (`EngineCapability`): slots, phases, `prompt_keys`, deal-seed /
   ...
   optional `display_name`. Override `default_system_template(phase)` when a phase needs
   built-in copy (Dou Dizhu bidding lives in `engine/doudizhu/prompts.py`).
...
Experiment detail exposes hypothesis/conclusion editing, comparison and control creation at every stage. Training remains optional under Analyze. The existing notebook stores editable text; it is not a versioned evidence snapshot. Review links explicitly use `train_usable=all` so structurally invalid decisions remain visible. Control creation defaults to source seats and includes all registered configurations without preferring `lora_*`. `collect=1` opens the confirmation dialog only; one-shot `collect` and `open_control` query parameters are consumed. Only an explicit collect submission starts games. Comparison suggestions on a control include its source experiment. No database migration is required for this workflow.

### Comparison integrity and snapshots

`core/stats/comparison.py` builds the sole effective seed cohort used by comparison rows and detail delta statistics. Exclude every duplicate seed (even failed retries), incomplete outcomes and mismatched seat order; never pick a first/last row. `coverage` separates planned seeds from effective seeds and retains member game IDs and exclusion reasons. Accept only the current nested Task protocol (schema_version=2); reject other protocol formats without conversion. `protocol_review` checks frozen fields by seat and exact declared Solver paths; rules, scorer and deal-plan changes cannot be waived. The check is retrospective and descriptive, not preregistration or statistical significance. Detail without declarations uses `protocol_mismatch` to suppress unsupported conclusions.

`comparison_snapshots` is append-only through `ComparisonRepository`; schema initialization creates this independent table for both old and new databases. POST `/experiments/comparisons` recalculates on the server, saves a result with `metric_version=paired-cohort-v2`, and returns the snapshot shown by the UI. GET list is paginated; GET by ID never recomputes and has no dependency on experiments still existing. The saved JSON is a comparison record, not a full evidence/reproduction package. `ExperimentCompareView` must flag selection/declaration changes as stale, require recomparison before save, and restore `?snapshot=` without a compare request. All matrix deltas are relative to the first column; numeric maxima are not styled as winners.

Research conclusions use `research_revisions`, append-only per comparison with optimistic revision checks inside `BEGIN IMMEDIATE`. Freeze cited decision fields; source changes/deletion only affect live evidence status. Never silently overwrite a conclusion or rebase a draft after conflict. Only current nested Task protocols are accepted for comparison. The exported research report contains the comparison and selected evidence, not runnable dependencies or complete game archives.
