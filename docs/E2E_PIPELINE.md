...
4. 实验详情 **开始对照实验**（挑战选手 + 与引擎人数相同的基线）→ 继续对局；对照就绪后留在详情看结论（证据不足时主句是证据句）；需要完整矩阵再进 **对比实验**
...
在无 CUDA 的本机上验证「真 LoRA → 导出」路径，用于确认训练链路可用，不评估真实牌力；端到端目标 **≤ 5 分钟**。
...
3. 确认对话框后开始；观察 **训练实时面板**（CPU / 内存 / 进度）；可随时 **取消**
...
创建实验时选择 **基准测试** 发牌模式（`collect_mode: benchmark`）。系统从 `GET /api/v1/system/benchmark-seeds` 返回的 50 个固定发牌种子中按目标局数截取，开局时按局序使用同一 seed，不再随机发牌。

适用场景：两个或多个模型在相同牌面条件下对比胜率 / 延迟。实验详情阶段下方给出**本实验**的指标（地主胜率、解析、可训练、延迟、每局 Token 与固定种子覆盖率）；对照实验用相同发牌验证这些数字。相对对照的 Δ 与是否可下结论仍在结论阶段；完整矩阵在对比页。详情与对比页会显示「基准测试」徽章。报告不进侧栏「分析」。
