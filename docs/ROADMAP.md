...
不建平行的表与 API。`train_usable` / `parser_success` / `latency_p50_p95` /
`role:landlord` 已走 `ScorerRegistry`（2026-09-07）；`ev_loss` 实验级均值仍待。
计数仍由 `eval_aggregates` 供给，Scorer 只负责公式与覆盖 summary 字段。
...
- **MCP server**：创建实验 / 启动收集 / 查结论 / 拉决策点，让外部 agent 编排研究。
  FastAPI 已有全部接口，成本低。
- **研究助手草稿**：实验结束基于 delta + scenario_scores + 关键决策点生成 `conclusion` 草稿，
  用户确认后写入。**只做草稿**，verdict 仍由统计代码决定。
...
| 3 | 录制—重放 + Task/Solver/Scorer 显式化 —— **已完成 2026-09-07**：VCR + protocol v2 + `ScorerRegistry`（含 `role:landlord` / latency）；`ev_loss` 实验级 Scorer 仍待 | harness 成型 |
