export default {
  guide: {
    ...
    tocTitle: '目录',
    lookupTitle: '查阅',
    intro: 'CardLab 以「实验」为主线：观察决策、采集对局、微调小模型，再对照验证。',
    hero: {
      goPlayers: '去选手配置',
      goExperiments: '去实验列表',
    },
    ...
    diagrams: {
      ...
      detail: {
        ...
        items: [
          '还没开始：一句说明加一个「开始实验」。',
          '进行中：进度数字，按钮变成「观战」。',
          '对局结束后：可训练决策条数，按钮是「开始训练」。',
          '训练结束后：用相同发牌再跑，按钮是「开始对照实验」。',
    ...
    sections: {
      ...
      experimentDetail: {
        title: '实验详情页',
        body: '每次只回答一个问题，只给一个按钮。跟着当前按钮就能走完整条链路。',
        bullets: [
      ...
      compare: {
        ...
        bullets: ['对照用配对发牌后，详情页给出结论；完整矩阵在这一页。'],
      },
      metrics: {
        title: '指标怎么算',
  ...
  metricHint: {
    ...
    overallDelta: {
      ...
      formula: 'Δ = 本实验地主胜率 − 对照（或上一轮）实验地主胜率，单位百分点（pp）。详情页按当前实验视角计算。',
    },
    pairedDelta: {
