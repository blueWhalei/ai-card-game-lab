import type { Experiment, ExperimentDelta, ExperimentVerdictKey } from '@/api/experimentApi'

/**
 * The four phases of the experiment workbench. Each one answers a single
 ...
 */
export type ExperimentStageId = 'empty' | 'collecting' | 'harvest' | 'verdict'
...
export function experimentProgressParts(finished: number, target: number): ExperimentProgressParts {
...
export function formatExperimentProgress(
  ...
  const { shownFinished, target: safeTarget, extra } = experimentProgressParts(finished, target)
...
export function verdictHeadlineOf(delta: ExperimentDelta, gamesNeeded = 0): VerdictHeadline {
  ...
  if (reason === 'protocol_mismatch') return { key: 'evidence.protocolMismatch', isEvidence: true }
